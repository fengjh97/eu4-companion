...
贴在欧陆风云4旁边的常驻面板：每次存档自动刷新「年度战略简报」（上年复盘 / 局势 / 今年目标 / 注意 / 行动清单）+ **深度分析**（趋势走向 / 财政诊断 / 军力 / 外交威胁 / 扩张规划 / 5-10年路线 / 定论），下面能跟军师聊天决定下一步。每次存档会用本局**历史时间线**做趋势对比。后端用 **Claude Code headless (`claude -p`)** 当大脑，**不需要 API key**，用你现成的 Claude Code 订阅。简报和深度分析**并行生成**，快报先到。
